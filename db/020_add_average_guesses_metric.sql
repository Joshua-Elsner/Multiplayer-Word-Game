...
BEGIN
    ...
    IF db_current_shark_id IS NOT NULL THEN
        UPDATE players SET fish_eaten = fish_eaten + 1, weekly_fish_eaten = weekly_fish_eaten + 1 WHERE id = db_current_shark_id;
    END IF;

    -- Update Loser Guesses
...
BEGIN
    ...
    -- Apply Winner Updates (Now Includes Guesses!)
    UPDATE players
...
-- 5. Update Weekly Reset RPC 
CREATE OR REPLACE FUNCTION process_weekly_shark_reset()
RETURNS void LANGUAGE plpgsql SECURITY DEFINER AS $$
-- [Keep all previous code until the "5. RESET ALL WEEKLY STATS" block...]
