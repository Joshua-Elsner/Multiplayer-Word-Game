...
BEGIN
    -- Validations
    IF upper(new_secret_word) !~ '^[A-Z]{5}$' THEN RAISE EXCEPTION 'Invalid secret word. Must be exactly 5 letters.'; END IF;
    IF NOT EXISTS (SELECT 1 FROM dictionary WHERE word = upper(new_secret_word)) THEN RAISE EXCEPTION 'Invalid word. Not in dictionary.'; END IF;
    IF EXISTS (SELECT 1 FROM used_words WHERE word = upper(new_secret_word)) THEN RAISE EXCEPTION 'Word already used! You must pick a word that has never been played.'; END IF;
    ...
    IF upper(guessed_word) != db_secret_word THEN RAISE EXCEPTION 'TOO SLOW!!! The Shark may have already been defeated by someone else!'; END IF;
    IF winner_id = db_current_shark_id THEN RAISE EXCEPTION 'You are already the Shark!'; END IF;
    ...
    WHERE id = winner_id
    RETURNING sharks_evaded, weekly_sharks_evaded, weekly_guesses, all_time_guesses, weekly_puzzles_played, all_time_puzzles_played
    INTO w_evaded, w_w_evaded, w_w_guess, w_a_guess, w_w_puzzles, w_a_puzzles;
    ...
    -- Emit the exact new stats
    INSERT INTO game_events (event_type, player_id, payload) 
    VALUES (
        'NEW_SHARK', 
        winner_id,
        jsonb_build_object(
            'sharks_evaded', w_evaded,
            'weekly_sharks_evaded', w_w_evaded,
            'weekly_guesses', w_w_guess,
            'all_time_guesses', w_a_guess,
            'weekly_puzzles_played', w_w_puzzles,
            'all_time_puzzles_played', w_a_puzzles
        )
    );
END;
$$;
